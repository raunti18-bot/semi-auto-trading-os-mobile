...
def email_config_ready() -> tuple[bool, str]:
    ...
    port = str(get_secret_value("SMTP_PORT", "") or "").strip()
    user = str(get_secret_value("SMTP_USER", "") or "").strip()
    password = str(get_secret_value("SMTP_PASSWORD", "") or "").strip()
    from_email = str(get_secret_value("ALERT_FROM_EMAIL", user) or "").strip()
    ...
    if missing:
        return False, "メール送信には Streamlit secrets に次が必要です: " + ", ".join(missing)
    return True, "メール送信の設定を検出しました。"


def send_email_smtp(recipient_email: str, subject: str, body: str) -> tuple[bool, str]:
    ready, msg = email_config_ready()
    if not ready:
        return False, msg

    host = str(get_secret_value("SMTP_HOST", "")).strip()
    port = int(str(get_secret_value("SMTP_PORT", "465")).strip())
    user = str(get_secret_value("SMTP_USER", "")).strip()
    password = str(get_secret_value("SMTP_PASSWORD", "")).strip()
    from_email = str(get_secret_value("ALERT_FROM_EMAIL", user)).strip()
    ...
    try:
        if port == 465:
            ...
            with smtplib.SMTP(host, port, timeout=20) as server:
                ...
                server.login(user, password)
                server.send_message(message)
        return True, f"メールを送信しました: {recipient_email.strip()}"
    except Exception as exc:
        return False, f"メール送信に失敗しました: {exc}"


def get_latest_price(ticker: str) -> tuple[float | None, str]:
    if not str(ticker).strip():
        return None, "ティッカーが空です。"
    try:
        hist = yf.Ticker(str(ticker).strip()).history(period="5d", interval="1d", auto_adjust=False)
        closes = hist.get("Close")
        if closes is None or closes.dropna().empty:
            hist = yf.Ticker(str(ticker).strip()).history(period="1d", interval="1m", auto_adjust=False)
            closes = hist.get("Close")
        if closes is None or closes.dropna().empty:
            return None, "現在価格を取得できませんでした。"
        return float(closes.dropna().iloc[-1]), "OK"
    except Exception as exc:
        return None, f"現在価格の取得に失敗しました: {exc}"


def evaluate_alert_hits(current_price: float, entry: float, stop: float, target: float, side: str, notify_entry: bool, notify_stop: bool, notify_target: bool) -> list[str]:
    hits: list[str] = []
    side = (side or "Long").strip()

    if side == "Short":
        if notify_entry and current_price <= entry:
            hits.append("エントリーライン到達")
        if notify_stop and current_price >= stop:
            hits.append("損切りライン到達")
        if notify_target and current_price <= target:
            hits.append("利確ライン到達")
    else:
        if notify_entry and current_price >= entry:
            hits.append("エントリーライン到達")
...
def build_trade_alert_email(symbol: str, side: str, current_price: float, entry: float, stop: float, target: float, hits: list[str]) -> tuple[str, str]:
    subject = f"[Trading OS] {symbol} 通知: {' / '.join(hits)}"
    body = "\n".join(
        [
            "Trading OS メール通知",
            "",
...
def build_scanner_email_message(df: pd.DataFrame, top_n: int = 3) -> tuple[str, str]:
    subject = f"[Trading OS] スキャナー候補 {min(top_n, len(df))}件"
    lines = ["Trading OS スキャナー候補", ""]
    for _, row in df.head(top_n).iterrows():
        lines.append(
            f"{row['Ticker']} | {row['Signal']} | score {row['Score']} | entry {row['Entry']} | stop {row['Stop']} | target {row['Target']} | RR {row['RR']}"
        )
    lines.append("")
    lines.append(f"送信時刻: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    return subject, "\n".join(lines)
...
def main() -> None:
    ...
    with st.expander("使い方", expanded=False):
        st.markdown(
            ...
            4. 必要なら Discord / メールへ候補を通知します。  
            5. プランでは Entry / Stop / Target 到達をメール判定できます。  
            """
        )
    ...
    with tabs[0]:
        ...
        st.subheader("メール通知（価格到達チェック）")
        alert_email = st.text_input("通知先メールアドレス", key="alert_email_recipient", placeholder="you@example.com")
        a1, a2, a3 = st.columns(3)
        notify_entry = a1.checkbox("Entry到達", value=True, key="alert_entry_hit")
        notify_stop = a2.checkbox("Stop到達", value=True, key="alert_stop_hit")
        notify_target = a3.checkbox("Target到達", value=True, key="alert_target_hit")

        email_ready, email_msg = email_config_ready()
        if email_ready:
            st.caption("SMTP secrets を検出しました。現在価格チェック時にメール送信できます。")
        else:
            st.info("メール送信を使うには Streamlit secrets に SMTP 情報を設定してください。README に設定例があります。")

        e1, e2 = st.columns(2)
        if e1.button("現在価格をチェックしてメール通知"):
            if not str(alert_email).strip():
                st.warning("通知先メールアドレスを入力してください。")
            elif not email_ready:
                st.error(email_msg)
            else:
                current_price, price_msg = get_latest_price(symbol)
                if current_price is None:
                    st.error(price_msg)
                else:
                    hits = evaluate_alert_hits(
                    ...
                    if not hits:
                        st.info("現在価格は通知条件に未到達です。")
                    else:
                        subject, body = build_trade_alert_email(symbol, side, float(current_price), float(entry_price), float(stop_price), float(target_price), hits)
                        ok, msg = send_email_smtp(str(alert_email), subject, body)
                        if ok:
                            st.success(msg)
                        else:
                            st.error(msg)

        if e2.button("このプランをテストメール送信"):
            if not str(alert_email).strip():
                st.warning("通知先メールアドレスを入力してください。")
            elif not email_ready:
                st.error(email_msg)
            else:
                subject, body = build_trade_alert_email(symbol, side, float(entry_price), float(entry_price), float(stop_price), float(target_price), ["テスト送信"])
                ok, msg = send_email_smtp(str(alert_email), subject, body)
                if ok:
                    st.success(msg)
                else:
    ...
    with tabs[1]:
        ...
        if scan_df is not None and not scan_df.empty:
            ...
            scan_email = st.text_input("候補通知先メール（任意）", value="", key="scan_email_recipient", placeholder="you@example.com")
            if st.button("上位候補をメール送信"):
                if not str(scan_email).strip():
                    st.warning("通知先メールアドレスを入力してください。")
                else:
                    email_ready, email_msg = email_config_ready()
                    if not email_ready:
                        ...
                        subject, body = build_scanner_email_message(scan_df, top_n=3)
                        ok, msg = send_email_smtp(str(scan_email), subject, body)
                        if ok:
                            st.success(msg)
                        else:
                            st.error(msg)
    ...
    with st.expander("この版で追加したもの", expanded=False):
        st.markdown(
            ...
            - 候補をワンタップでプランへ反映
            - Discord Webhook 通知
            - SMTPメール通知（候補送信 / Entry・Stop・Targetチェック）
            - セッション中のスキャン結果保持
            - スキャナー用チャート表示
            """
        )
