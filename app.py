...
def build_chart(
    ...
    if time_stop_bars > 0 and len(df) >= 2:
        ...
        target_date = pd.to_datetime(df["Date"]).iloc[-1] + (step * time_stop_bars)
        fig.add_vline(
            x=target_date, line_color="orange", line_dash="dash", annotation_text="Time Stop", annotation_position="top", row=1, col=1
        )
...
def preload_from_scan(row: pd.Series) -> None:
    st.session_state["planner_symbol"] = row["Ticker"]
    st.session_state["planner_entry"] = float(row["Entry"])
    st.session_state["planner_stop"] = float(row["Stop"])
    st.session_state["planner_target"] = float(row["Target"])
